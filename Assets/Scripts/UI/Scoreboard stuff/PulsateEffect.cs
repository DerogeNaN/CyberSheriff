...
public class PulsateEffect : MonoBehaviour
{
    ...
    public float pulsateScale = 1.2f;  // Scale factor for pulsating
    ...
    public void StartPulsateAndShake()
    {
        if (!isPulsating)
        {
            StartCoroutine(PulsateAndShake());
        }
    }

    private IEnumerator PulsateAndShake()
    {
        isPulsating = true;
